...
export default function Sidebar({
  ...
  const getRoom = async () => {
    ...
    try {
      const response = await fetch(`${baseUrl}/rooms/${activeServerId}`, options("GET", tokenString));
      const data = await response.json();
      // Guard against non-array responses to prevent `.map is not a function` error
    ...
    <aside className={`sidebar ${isOpen ? "open" : ""}`}>
        ...
        <div className="sidebar-content">
            ...
            {activeServer ? (
              <>
                ...
                <div style={{ position: "relative", display: "flex", alignItems: "center", gap: "4px" }}>
                  ...
                  <button onClick={handleInvite} className="invite-button" aria-label="Invite" title="Invite people">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  {showInviteModal && (
                    <div style={{
                      ...
                      borderRadius: 6, fontSize: 12, fontWeight: 600, whiteSpace: "nowrap", zIndex: 50
                    }}>
                      Copied!
                    </div>
                  )}
              ...
              <div className="brand" style={{ color: "var(--muted)" }}>No Server</div>
            )}
          </div>
          ...
          <div className="channel-list">
            {!activeServer ? (
              ...
              <div style={{ padding: "20px", textAlign: "center", color: "var(--muted)", fontSize: 13 }}>
                {query ? "No channels match your search." : "No channels in this server."}
              </div>
            ) : (
              filteredRooms.map((room) => (
                ...
                >
                  <div style={{ flex: 1, cursor: "pointer", display: "flex", alignItems: "center" }} onClick={() => onSelectRoom(room.name, room.id)}>
      ...
      {show && (
        <InputModal
          ...
          fields={[
            { name: "name", label: "Channel Name", placeholder: "e.g. general", required: true, type: "text" },
            { name: "description", label: "Topic (optional)", placeholder: "What's this channel about?", type: "text" },
          ]}
      ...
      {activeServer && (
        <ServerSettingsModal
          ...
          onServerDeleted={() => {
            getServer();
            // Optional: You could navigate away or clear the active server if needed
            if (typeof onServerChange === "function") onServerChange("");
          }}
        />
      )}
      {settingsRoom && (
        <ChannelSettingsModal
          ...
          onRoomDeleted={() => {
            getRoom();
            if (activeRoomName === settingsRoom.name) {
